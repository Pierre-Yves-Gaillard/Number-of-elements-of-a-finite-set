% cardfin-a
% https://docs.google.com/document/d/1_yFircUDb5kcQiElNGR8y7z2UQbdNEkbDI0g-8mXudI/edit?tab=t.0
% !TEX encoding = UTF-8 Unicode
% https://www.site24x7.com/tools/time-stamp-converter.html 1767024737
\documentclass[12pt,letterpaper]{article}
\usepackage{fancyhdr}
\fancyhf{}
\fancyfoot[R]{{\tiny cardfin-a,\ \filemodprintdate{\jobname},\ \filemodprinttime{\jobname},\ 1767024737}} % https://www.site24x7.com/tools/time-stamp-converter.html (https://www.unixtimestamp.com/ bad)
...
Let $\N$ be the set of nonnegative integers, and, for each $n\in\N$ set $[n]:=\{1,2,\ldots,n\}$ (in particular $[0]=\varnothing$). If $X$ and $Y$ are two sets, write $X\simeq Y$ to indicate that there is a bijection $X\to Y$. If $X\simeq[n]$ for some $n\in\N$ say that $X$ is \textbf{finite}. 

\begin{thm}\label{T}
If a set $X$ is finite, then there is a \emph{unique} $n\in\N$ such that $X\simeq[n]$. 
\end{thm} 

If such is the case, we say that $X$ \textbf{has} $n$ \textbf{elements}. The theorem will follow from the proposition below: 

\begin{prop}\label{P}
If $m,n\in\N$ satisfy $[m]=[n]$, then we have $m=n$. 
\end{prop} 
...
Let $m,n\in\N$ satisfy $[m]\simeq[n]$. We must show $m=n$. We can assume $m\ge n$. Assume by contradiction that $m\ne n$ and that $n$ is minimal for the conditions $[m]=[n]$ and $m>n$. If $n=0$, then there is no map $[m]\to[n]$, so this case is impossible. If $m>n>0$, then Lemma~\ref{Lc} implies $[m-1]\simeq[n-1]$, contradicting the minimality of $n$. 
\end{proof} 
